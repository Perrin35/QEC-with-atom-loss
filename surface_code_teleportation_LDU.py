...
class Rotated_Surface_Code_teleportation_LDU:
    """
    Class to construct the circuit corresponding to the surface code  with the effective noise mimicking the action of the teleportation LDU
    ...
    def __init__(
        ...
        Args:
            ...
            loss_rate(Float): Default to 0. Loss rate per CZ gate.
    ...
    @staticmethod
    def fidelities_conjugate_HH(fidelities: np.ndarray) -> np.ndarray:
        """
        Compute the resulting fidelities after conjugating with Hadamard gates.
        ...
        """
        fid_conj_HH=fidelities.copy()
        for i,f in enumerate(fidelities):
            if i%4==1:
                fid_conj_HH[i]=fidelities[i+3]
                fid_conj_HH[i+3]=fidelities[i]

        for i,f in enumerate(fidelities):
            if i//4==1:
                fid_conj_HH[i]=fidelities[i+8]
                fid_conj_HH[i+8]=fidelities[i]
                
        return fid_conj_HH
    
        

    def effective_noise_detection_circuit(self) -> Optional[Tuple[str, Any]]:
        """
        Compute the effective noise due to the teleportation LDU.

        Returns:
            Optional[Tuple[str, Any]]: Effective noise model and its parameters.
        """
        if self.after_CZ!=None:
            ...
            if error_model=='DEPOLARIZE2':
                ...
                final_fidelities=self.fidelities_conjugate_HH(fidelities)
                pauli_noise2=self.fidelities_to_pauli_noise(final_fidelities)
                x_error=sum([pauli_noise2[1],pauli_noise2[6],pauli_noise2[10],pauli_noise2[13]])
                y_error=sum([pauli_noise2[2],pauli_noise2[5],pauli_noise2[9],pauli_noise2[14]])
                z_error=sum([pauli_noise2[3],pauli_noise2[4],pauli_noise2[8],pauli_noise2[15]])
                
                return ('PAULI_CHANNEL_1',[x_error,y_error,z_error])   
    ...
    def effective_detection_loss_circuit(self) -> stim.Circuit:
        """
        Generate the effective noise associated to the teleportation LDU.

        Returns:
            stim.Circuit: Circuit for simulating the effective noise of the teleporation LDU.
        """
        n_data=self.n_data
        indices_data=[self.index_data_qubit(i) for i in range(n_data)]
        circuit = stim.Circuit()
        ...
        if self.after_CZ!=None:
            effective_noise,effective_proba=self.effective_noise_detection_circuit()
