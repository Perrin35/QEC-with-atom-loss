...
from sampler_teleportation_LDU import sampler_teleportation_LDU
import pymatching
import warnings
import pickle
...
class DEM_builder:
    ...
    def atom_loss_circuit_without_noise(self,atoms_lost_position=None):
        ...
        if atoms_lost_position==None:
            ...
            position_qubits = [0 for _ in range(len(np_atoms_lost_position))]  # Tracks qubit positions  in the round
    ...
    def build_dem_lost_data_qubits(self,save_path):
        ...
        for qubit,round_position in positions_data:
            flip_obs=self.flip_observable(qubit,round_position)# Determine if observable flips occur.
    ...
    def build_dem_lost_qubits(self,position,path_dem):
        ...
        """
        lattice=self.lattice
        dem=stim.DetectorErrorModel()# Initialize an empty DEM.
