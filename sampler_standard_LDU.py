...
import pickle
import re
class sampler_standard_LDU:
    """
    Class to sample atom loss (and standard Pauli error) in the rotated surface code equipped with the standard LDU
    ...
    def detection_error(self) -> float:
        """
        Computes the probability to detect a loss that did not happen due to to the noise on the CZ gates
    
        Returns:
            float: The detection error probability.
    
        Raises:
            NotImplementedError: If the noise model is unsupported.
        """
        lattice=self.lattice
        if lattice.after_CZ!=None:
            error_model,proba=lattice.after_CZ
        else:
            return 0
        ...
        if error_model=='DEPOLARIZE2':
            ...
            fidelities=lattice.pauli_noise_to_fidelities(pauli_noise)
            fid_conj_X=lattice.fidelities_conjugate_X2(fidelities)
            fid_conj_CZ=lattice.fidelities_conjugate_CZ(fid_conj_X)
            final_fidelities=np.array(fidelities)*np.array(fid_conj_CZ)
            pauli_noise2=lattice.fidelities_to_pauli_noise(final_fidelities)
            det_err_model,proba_error=lattice.trace_noise_on_1qubit(('PAULI_CHANNEL_2',pauli_noise2[1:]),qubit=0)
            return proba_error[1]+proba_error[2]


    def no_detection_error(self)-> float:
        """
        Computes the probability to not detect a loss due to to the noise on the CZ gates
        ...
        lattice=self.lattice
        if lattice.after_CZ!=None:
            CZ_1qubit=lattice.trace_noise_on_1qubit(lattice.after_CZ,qubit=0)
        else:
            return 0.
        if CZ_1qubit[0]=='DEPOLARIZE1':
            fe=(1-4*CZ_1qubit[1]/3)**2
            return (1-fe)/2
    ...
    def atoms_lost_position_list(self,num_shots):
        ...
        err_det=self.detection_error()
        err_no_det=self.no_detection_error()


        # Generate probabilities for different qubit groups
        # Corner data probabilities
        proba_corner_data_standard_CZ=[error_rate*(1-error_rate)**t for t in range(2)]
        proba_corner_data_detection_CZ=[error_rate*(2-error_rate-(1-error_rate)**3)*(1-error_rate)**2/(1-(1-error_rate)**2+(1-error_rate)**4), error_rate*(1-error_rate)**5/(1-(1-error_rate)**2+(1-error_rate)**4)]
        p_tot_corner=1-sum(proba_corner_data_standard_CZ)-sum(proba_corner_data_detection_CZ) #probability to not lose the data qubit
        proba_corner_data_list=[p_tot_corner*(1-err_det)] + [p*(1-err_no_det) for p in proba_corner_data_standard_CZ] + [p_tot_corner*err_det+proba_corner_data_detection_CZ[0]*(1-err_no_det) + proba_corner_data_detection_CZ[1]/2]+[p*err_no_det for p in proba_corner_data_standard_CZ] + [proba_corner_data_detection_CZ[0]*err_no_det + proba_corner_data_detection_CZ[1]/2]
        rand_corner_data=np.random.RandomState().choice([i for i in range(7)], (num_shots,4,rounds-1), p=proba_corner_data_list)

        # Edge data probabilities
        proba_edge_data_standard_CZ=[error_rate*(1-error_rate)**t for t in range(3)]
        proba_edge_data_detection_CZ=[error_rate*(2-error_rate-(1-error_rate)**3)*(1-error_rate)**3/(1-(1-error_rate)**2+(1-error_rate)**4), error_rate*(1-error_rate)**6/(1-(1-error_rate)**2+(1-error_rate)**4)]
        p_tot_edge=1-sum(proba_edge_data_standard_CZ)-sum(proba_edge_data_detection_CZ) #probability to not lose the data qubit
        proba_edge_data_list=[p_tot_edge*(1-err_det)] + [p*(1-err_no_det) for p in proba_edge_data_standard_CZ] + [p_tot_edge*err_det+proba_edge_data_detection_CZ[0]*(1-err_no_det) + proba_edge_data_detection_CZ[1]/2]+[p*err_no_det for p in proba_edge_data_standard_CZ] + [proba_edge_data_detection_CZ[0]*err_no_det + proba_edge_data_detection_CZ[1]/2]
        rand_edge_data=np.random.RandomState().choice([i for i in range(9)], (num_shots,2*(dw-2)+2*(dh-2),rounds-1), p=proba_edge_data_list)

        # Bulk data probabilities
        proba_bulk_data_standard_CZ=[error_rate*(1-error_rate)**t for t in range(4)]
        proba_bulk_data_detection_CZ=[error_rate*(2-error_rate-(1-error_rate)**3)*(1-error_rate)**4/(1-(1-error_rate)**2+(1-error_rate)**4), error_rate*(1-error_rate)**7/(1-(1-error_rate)**2+(1-error_rate)**4)]
        p_tot_bulk=1-sum(proba_bulk_data_standard_CZ)-sum(proba_bulk_data_detection_CZ) #probability to not lose the data qubit
        proba_bulk_data_list=[p_tot_bulk*(1-err_det)] + [p*(1-err_no_det) for p in proba_bulk_data_standard_CZ] + [p_tot_bulk*err_det+proba_bulk_data_detection_CZ[0]*(1-err_no_det) + proba_bulk_data_detection_CZ[1]/2]+[p*err_no_det for p in proba_bulk_data_standard_CZ] + [proba_bulk_data_detection_CZ[0]*err_no_det + proba_bulk_data_detection_CZ[1]/2]
        rand_bulk_data=np.random.RandomState().choice([i for i in range(11)], (num_shots,(dw-2)*(dh-2),rounds-1), p=proba_bulk_data_list)
        ...
        for corners_data , last_round_corners_data , edges_data , last_round_edges_data , bulk_data , last_round_bulk_data , edges_ancilla , bulk_ancilla in zip( rand_corner_data , last_round_rand_corner_data , rand_edge_data , last_round_rand_edge_data , rand_bulk_data , last_round_rand_bulk_data , rand_edge_ancilla , rand_bulk_ancilla):
            position_temp=[]
            ...
            for i,positions in enumerate(corners_data):
                ...
                for ro,position in enumerate(positions):
                    if undetected_lost==1:
                        position_temp.append((corner_indices_data[i],ro,-1))
                        b=np.random.RandomState().binomial(1, err_no_det)
                        if b==0:
                            undetected_lost=0
                    elif position<4 and position>0:
                        position_temp.append((corner_indices_data[i],ro,position-1))
                    elif position>=4:
                        position_temp.append((corner_indices_data[i],ro,position-4))
                        undetected_lost=1
            ...
            for i,positions in enumerate(edges_data):
                ...
                for ro,position in enumerate(positions):
                    if undetected_lost==1:
                        ...
                        if b==0:
                            undetected_lost=0
                    elif position<5 and position>0:
                        position_temp.append((edge_indices_data[i],ro,position-1))
                    elif position>=5:
                        position_temp.append((edge_indices_data[i],ro,position-5))
                        undetected_lost=1
            ...
            for i,positions in enumerate(bulk_data):
                ...
                for ro,position in enumerate(positions):
                    if undetected_lost==1:
                        ...
                        position_temp.append((bulk_indices_data[i],ro,position-6))
                        undetected_lost=1
    ...
    def atom_loss_circuit(self,atoms_lost_position=None):
        """
        ...
        Returns:
            stim.Circuit: The modified circuit with with operations suppressed for lost qubits
        ...
        # If no atom loss information is provided, return the original circuit.
        if atoms_lost_position==None:
            ...
            qubits_lost_now=[]
            det=0
            position_qubits=[0 for i in range(len(np_atoms_lost_position))]#current time position of each qubits which have been lost
    ...
    def modify_detector_error_model(self,atoms_lost_position=None):
        ...
        Returns:
            stim.Circuit: A modified circuit reflecting the changes in detector error models due to ancilla losses.
    ...
    def sampler(self, position):
        ...
        """
        # Step 1: Generate a modified circuit based on qubit losses.
        # This suppresses operations on lost qubits to the circuit.
        new_circuit=self.atom_loss_circuit(position)
